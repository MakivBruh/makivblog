<x-layout :title='$title'>
    ...
    {{-- <article class="py-8 max-w-screen-md">
        ...
        <div class="text-base text-gray-500">
            <a class="hover:underline" href="/authors/{{ $post->author->username }}">{{ $post->author->name }}</a> | {{ $post['date'] }}
        </div>
            ...
            <header class="my-4 lg:mb-6 not-format">
                <address class="flex items-center mb-6 not-italic">
                    <div class="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white">
                        <img class="mr-4 w-16 h-16 rounded-full" src="{{ $post->author->avatar ? asset('storage/' . $post->author->avatar) : asset('img/avatar_default.jpg') }}" alt="{{ $post->author->name }}">
                        <div>
                            <a class="hover:underline" href="/posts?author={{ $post->author->username }}" rel="author" class="text-xl font-bold text-gray-900 dark:text-white">{{ $post->author->name }}</a>
                            <a class="block" href="/posts?category={{ $post->category->slug }}">
